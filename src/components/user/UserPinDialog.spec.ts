...
import UserPinDialog from './UserPinDialog.vue'
...
function saveButton(): HTMLButtonElement {
  const button = document.body.querySelector<HTMLButtonElement>('[data-testid="user-pin-save-button"]')
...
describe('UserPinDialog', () => {
  it('save sonrası kendini kapatmaz — update:visible false yayınlanmaz', async () => {
    // Arrange
    const wrapper = mountDialog()
    await nextTick()
    setInput('user-pin-new', '1234')
    setInput('user-pin-new-confirm', '1234')
    await nextTick()
  ...
  it('saving true iken form alanları devre dışı kalır', async () => {
    ...
    // Assert
    expect(document.body.querySelector<HTMLInputElement>('#user-pin-new')?.disabled).toBe(true)
  })
  ...
  it('iptal her zaman kapatır', async () => {
    ...
    // Act
    document.body.querySelector<HTMLButtonElement>('[data-testid="user-pin-cancel-button"]')!.click()
    await nextTick()
  ...
  it('PIN tekrarı uyuşmazsa kaydetmez, Türkçe uyarı gösterir', async () => {
    ...
    await nextTick()
    setInput('user-pin-new', '1234')
    setInput('user-pin-new-confirm', '5678')
    await nextTick()
